...
interface MenuPaymentFormProps {
  itemName: string;
  totalAmount: number;
...
const MenuPaymentForm: React.FC<MenuPaymentFormProps> = ({ itemName, totalAmount, onPaymentComplete, onCancel }) => {
  ...
  const savePaymentData = (data: any) => {
    const paymentData = {
      ...data,
    ...
    localStorage.setItem(`menu_payment_${Date.now()}`, paymentDataString);
  };
  ...
  return (
    <PaymentContainer>
      <Title>Payment for {itemName}</Title>
...
export default MenuPaymentForm; 